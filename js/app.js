...
function renderCalendar() {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if(dayAppts.length)    classes.push('cal-cell--appt');
...
function renderSidebar() {
  ...
  const activeMeds = medications
    .filter((m) => m.active && statuses[m.id]?.runOut)
    .sort((a, b) => statuses[a.id].runOut.localeCompare(statuses[b.id].runOut));

  if (activeMeds.length === 0) {
    sidebar.innerHTML = '<p class="sidebar-empty">No medications tracked yet.</p>';
    return;
  }
...
function renderSettingsView() {
  const { settings, vacations } = App.state;
  const sortedVac = [...vacations].sort((a,b) => a.startDate.localeCompare(b.startDate));
  ...
  container.innerHTML = `
    ...
    <section class="settings-section">
      <h3>Vacation &amp; Absence Periods</h3>
      <p class="view-desc">Link each vacation to a doctor to limit which medications it affects. An unlinked vacation affects all medications.</p>
      <button class="btn btn--primary btn--sm" id="btn-add-vac" style="margin-bottom:1rem">+ Add period</button>
      <div class="vac-list">
        ${sortedVac.length === 0 ? '<p class="empty-hint">No vacation periods set.</p>'
          : sortedVac.map(vacRow).join('')}
      </div>
    </section>
    ...
    <div class="modal-body">
      ...
      <div id="daily-fields" class="${intakeType!=='daily'?'hidden':''}">
        ...
        <div id="variable-dose-field" class="${!variableDose?'hidden':''}">
          ...
          <p class="field-hint">A 10% conservative buffer is applied automatically.</p>
        </div>
      </div>
      ...
      <div id="prn-fields" class="${intakeType!=='prn'?'hidden':''}">
        ...
        <p class="field-hint">Leave 0 if too unpredictable — run-out estimate will be skipped.</p>
      </div>
...
async function exportData() {
  try {
    ...
    const a    = Object.assign(document.createElement('a'), { href: url, download: `medtracker-backup-${todayStr()}.json` });
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
    URL.revokeObjectURL(url);
    toast('Backup exported');
  } catch (err) {
    console.error(err); toast('Export failed', 'error');
  }
}
